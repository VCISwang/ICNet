...
from LANet.FCN_16s import FCN_res50 as FCN
...
# ASP module from: 'Multi-scale context aggregation for semantic segmentation of remote sensing images[J].
# Remote Sensing.'
class ASP(nn.Module):
...
# Patch attention module. Parameters: reduction is the rate of channel reduction.
# pool_window should be set according to the scaling rate.
class Patch_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        A = F.upsample(A, (h,w), mode='bilinear', align_corners=False)
        output = x*A
        if self.add_input:
            output += x
...
# Attention embedding module. Parameters: reduction is the rate of channel reduction.
# pool_window should be set according to the scaling rate.
...
class LANet(nn.Module):
    ...
    def forward(self, x):
        ...
        low = F.upsample(low, x_size[2:], mode='bilinear', align_corners=False)
        
        high = self.classifier1(x2)
        high = F.upsample(high, x_size[2:], mode='bilinear', align_corners=False)
